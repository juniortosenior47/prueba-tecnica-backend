...
public class MaterialBusiness implements MaterialApi {
    ...
    public ResponseEntity<APIResponses> listMaterial() {
        try {
            ...
            List<Material> lm = ApiUtil.toMaterialBy(lMDB, lCiDB, lCoDB);

            if (lm.isEmpty() || lm.size() == 0) {
                return this.responseNotFound(lm.stream().collect(Collectors.toList()));
            } else {
                return this.responseOk(lm.stream().collect(Collectors.toList()));
            }
